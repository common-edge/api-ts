...
import { boolTrue } from '~model/type-helpers';
import { Angle } from '~model/Numbers';
import { OneOrUptoThree } from '~model/Boxes';
import { Measurement, Directed, Location } from './Measurement';
 ...
 * @category Offset
 */
export const Offset: t.Type<Offset> =
	t.interface({
		Offset: t.union([t.number, t.null]),
		Location: Location,
	});
...
export const CurbEdge: t.Type<CurbEdge> =
	t.interface({
		Bredth: t.union([Measurement('In'), t.null]),
		Offset: t.union([Offset, t.null]),
		OuterHeight: t.union([t.number, t.null]),
	});
