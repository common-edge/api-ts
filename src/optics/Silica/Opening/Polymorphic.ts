...
import { idOpt } from '~optics/Common';

import { Opening, Wall, SectionTree, Section } from '~model/Silica/Opening/Polymorphic';
import { OneOrUptoThree } from '~model/Boxes';
import { uptoThree } from '~optics/Boxes';
...
/**
 * Focus on a child `SectionTree` of a `SectionTree`, given it's index.
 *
 * @since 0.2.0
 * @category SectionTree
 */
export const sectionTree = <A,B,C>(is: SectionStep[]): Optional<SectionTree<A,B,C>, SectionTree<A,B,C>> =>
...
export const sectionSection = <A,B,C>(is: SectionStep[]): Optional<SectionTree<A,B,C>, Section<A,B,C>> =>
	sectionTree<A,B,C>(is).compose(sectionTreeSection<A,B,C>().asOptional());
